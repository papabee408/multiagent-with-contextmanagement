...
TRACKED_RECEIPT_FILE="$(tracked_scope_review_receipt_file "$TASK_ID")"
...
replace_key_value_or_exit "$TASK_FILE" "## Review Status" "scope-review-fingerprint" "$(task_fingerprint "$TASK_ID")"
replace_key_value_or_exit "$TASK_FILE" "## Status" "state" "review"
touch_task_updated_at "$TASK_ID"
replace_key_value_or_exit "$TASK_FILE" "## Session Resume" "current focus" "scope review recorded; finish remaining review or complete the task"
replace_key_value_or_exit "$TASK_FILE" "## Session Resume" "next action" "run bash scripts/review-quality.sh $TASK_ID if quality review is still pending"
